...
public class ConnectionUtil {
    ...
    public static DatabaseMetaData createThisDatabaseMetaData(){
        try {
            return connectionTl.get().getMetaData();
        } catch (Exception e) {
            throw new RuntimeException("创建 DatabaseMetaData 发生异常", e);
        }
    }
    ...
    public static void init(CodeConfig codeConfig) {
        try {
            Class.forName(codeConfig.getDriver());
            connectionTl.set(DriverManager.getConnection(codeConfig.getUrl(), codeConfig.getUserName(), codeConfig.getPassword()));
        } catch (Exception e) {
            throw new RuntimeException("创建 Connection 发生异常", e);
        }
    }
